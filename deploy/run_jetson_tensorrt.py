...
class TRT_YOLO_Seg:
    def __init__(self, engine_path, input_shape=(640, 640)):
        ...
        with open(engine_path, "rb") as f, trt.Runtime(TRT_LOGGER) as runtime:
            self.engine = runtime.deserialize_cuda_engine(f.read()) # 反序列化生成 TensorRT 引擎对象
        self.context = self.engine.create_execution_context() # 创建执行上下文，用于后续推理调用
        ...
        for binding in self.engine:
            # 获取输入输出的形状、大小和数据类型，以便后续分配正确大小的显存
            ...
            # 分配主机端与设备端(GPU)内存
            host_mem = cuda.pagelocked_empty(size, dtype) # page-locked 内存，提升 CPU-GPU 传输效率
            device_mem = cuda.mem_alloc(host_mem.nbytes) # 在 GPU 上分配内存
            self.bindings.append(int(device_mem)) # 记录绑定地址，后续推理调用需要用到
...
def main():
    engine_path = "best.engine"  # 建议把上一步在Nano上生成的engine放到同目录
    imgsz = (640, 640)  # 这个尺寸必须和你生成engine时的输入尺寸一致，否则推理结果会完全错乱
    ...
    while True:
        ...
        num_mask_coeffs = 32 # 这个值必须和你训练时 Mask 头的输出通道数一致，否则后续维度解析会完全错乱
