...
加载 -seg 权重并在已被 SAM 处理好的包含 Mask Txt 标注的训练集上实现 O(1) 的前向推理。
"""
import sys
...
def main():
    ...
    # 这里直接使用已经下载好的权重
    model = YOLO(str(PROJECT_ROOT / 'yolov8n-seg.pt'))

    yaml_path = str(PROJECT_ROOT / 'CottonWeedDet12' / 'Dataset' / 'data.yaml')
