...
def main():
    """农业机器人视觉流水线 (端到端高速单模型方案: YOLOv8n-seg)"""
    
    # 需要先训练得到模型，路径在 runs/segment/train... 下
    # 当导出为 tensorrt 之后，也可以将这里换成 .engine
    yolo_weight = "runs/segment/train_aug_only/weights/best.pt"
    image_path = "CottonWeedDet12/Dataset/images/test/20210712_iPhoneSE_YL_213.jpg"
    ...
    torch.cuda.synchronize() # 强制同步，确保预热完成后再进行正式推理测试
    # ================================
    ...
    torch.cuda.synchronize() # 强制同步，确保推理完成后再进行时间统计
    ...
    print(f"[INFO] 成功预估出 {len(boxes)} 个目标的 BBox 与 Mask。")
    ...
    if results.masks is not None:
        ...
        for idx in range(len(boxes)):
            ...
            # 因为使用了 retina_masks=True，尺寸已经对齐，但为了安全依然统一进行 Resize 核对
